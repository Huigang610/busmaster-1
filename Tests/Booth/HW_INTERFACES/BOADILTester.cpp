...
// BOADILTester.cpp : Defines the initialization routines for the DLL.
//
...
BOOST_AUTO_TEST_SUITE( BOA_DIL_Tester )
...
{
	CString omCurrExe;
    omCurrExe.Format("%s\\BOA Controller Configuration.exe", g_strPath);
...
BOOST_AUTO_TEST_CASE( DIL_Negative_Test_Cases)
{
	...
	g_pBaseDILCAN_Controller->CAN_SendMsg(g_dwClientID, sMsg);
	g_pBaseDILCAN_Controller->CAN_StopHardware();
	g_pBaseDILCAN_Controller->CAN_GetErrorCount( sErrorCounter, 10, ERR_CNT);
